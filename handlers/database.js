...
module.exports = async client => {
	...
	client.con.connect(err => {
		...
		let settings = client.query("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 's224_shotoDev' AND table_name = 'Settings'");
		let users = client.query("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 's224_shotoDev' AND table_name = 'Users'");
