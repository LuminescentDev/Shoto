module.exports = {
	...
	execute(client, interaction, args){
		try {
			client.con.query(`UPDATE Settings Set joinMessage = "${args[1]}" where guildID = "${interaction.guild.id}"`);
			interaction.editReply(`Setting: Join Message Updated to ${args[1]}`);
		} catch (error) {
