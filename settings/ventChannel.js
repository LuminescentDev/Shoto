module.exports = {
	...
	execute(client, interaction, args){
		try {
			if(!interaction.guild.channels.cache.get(args[1]) || !interaction.guild.channels.cache.get(args[1]).type === "GUILD_TEXT")return interaction.reply({content: "Channel is either not from this guild or invalid please try again", ephemeral: true});
			client.con.query(`UPDATE Settings Set ventChannelID = "${args[1]}" where guildID = "${interaction.guild.id}"`);
			interaction.reply(`Setting: Vent Channel Updated to ${args[1]}`);
		} catch (error) {
