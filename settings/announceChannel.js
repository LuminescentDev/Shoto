module.exports = {
	...
	execute(client, interaction, args){
		try {
			if(!interaction.guild.channels.cache.get(args[1]) || !interaction.guild.channels.cache.get(args[1].type === "GUILD_TEXT") || interaction.guild.channels.cache.get(args[1].type === "GUILD_NEWS"))return interaction.editReply({content: "Channel is either not from this guild or invalid please try again",  ephemeral: true});
			client.con.query(`UPDATE Settings Set AchannelID = "${args[1]}" where guildID = "${interaction.guild.id}"`);
			interaction.editReply(`Setting: Announcement Channel Updated to ${args[1]}`);
		} catch (error) {
